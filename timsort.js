...
// replacing default Array.sort with timsort
...
Array.prototype.sort = function(comp){
  ...
  //  console.log("timsort?? " + JSON.stringify(this) + comp)
    sort(this,0,this.length,compare);
 //   console.log("timsorted " + JSON.stringify(this))
    ...
    function sort (a, lo, hi, compare) {
        ...
        if (nRemaining < MIN_MERGE) {
            let initRunLen = countRunAndMakeAscending(a, lo, hi, compare);
        ...
        do {
            // Identify next run
            let runLenlet = countRunAndMakeAscending(a, lo, hi, compare);
